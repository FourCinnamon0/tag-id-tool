...
function tag2id(hashtag){
	...
	for(var i = 0;i < tagArray.length;i++){
		...
		if(charIndex == -1){console.error(`Invalid Tag! Tags can only contain the following characters: ${tagChar.join(", ")}`);process.exit(1)}
    	id *= tagChar.length
	    id += charIndex
	}
	return id
...
function doThings(type,input) {
	...
	switch (type) {
		case "tag2id":case "tag2hl":case "tag":
			if(!input){
				console.error("No tag specified!")
				if(!isInt){process.exit()}else{return;}
			}
			var gid = tag2id(input)
			console.log("ID:    "+gid)
			console.log("HL ID: "+id2hl(gid).join("-"))
			break;
		case "hl2id":case "hl2tag":case "hl":
			if(!input){
			...
			console.log("ID:     "+gid)
			break;
		case "id2hl":case "id2tag":case "id":
			if(!input){
				console.error("No HL ID specified!")
				if(!isInt){process.exit()}else{return;}
			}
